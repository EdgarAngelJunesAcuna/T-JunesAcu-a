		...
		<div class="card" id="divRegistro" style="display: none;">
			...
			<div class="card-body">
				<form class="needs-validation" id="managerForm" novalidate>
					...
					<div class="row mb-3">
						<label for="frmDocument_number" class="col-sm-2 col-form-label">DNI</label>
						<div class="col-md-6 mb-3">
							<input type="text" class="form-control" id="frmDocument_number"
								name="frmDocument_number" required pattern="[0-9]{8}"
								maxlength="8">
							<div class="valid-feedback">Todo bien</div>
							<div class="invalid-feedback">Es necesario el número de
								documento DNI (8 dígitos)</div>
						</div>
					</div>

					<script>
						// Obtener el campo de entrada de DNI
						var dniInput = document
								.getElementById('frmDocument_number');

						// Agregar un event listener para el evento 'input'
						dniInput.addEventListener('input', function(event) {
							// Obtener el valor actual del campo de DNI
							var dni = event.target.value;

							// Expresión regular para validar exactamente 8 dígitos numéricos
							var regex = /^[0-9]{8}$/;
