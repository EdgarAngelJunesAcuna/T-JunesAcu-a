...
<!-- Llamar automaticamente la lista de Encargado -->
...
<body>
			...
			<div class="card-body">
				<form method="post" action="managerBuscar">
					<div class="mb-3 row">
						...
						<div class="col-sm-2">
							<button type="button" class="btn btn-primary mb-3" id="btnListar"
								name="btnListar">Listar Inactivos</button>
						</div>

						<div class="col-sm-2">
							<button type="button"
								class="btn btn-outline-primary mb-3 btn-generar-pdf"
								id="btnGenerarPDF" name="btnGenerarPDF">Generar PDF</button>
						</div>
						<div class="col-sm-2">
							<button type="button"
								class="btn btn-outline-primary mb-3 btn-generar-csv"
								id="btnGenerarCSV" name="btnGenerarCSV">Generar CSV</button>
						</div>
						<div class="col-sm-2">
							<button type="button"
								class="btn btn-outline-primary mb-3 btn-generar-xls"
				...
				<form class="needs-validation" id="managerForm" novalidate>
					...
					<script>
						...
						nombresInput.addEventListener('input', function(event) {
							...
							// Expresión regular para validar solo letras, espacios, tildes y la letra "ñ"
					...
					<div class="row mb-3">
						...
						<div class="col-md-6 mb-3">
							...
							<div class="valid-feedback">Todo bien</div>
							<div class="invalid-feedback">Solo puedes ingresar números en el DNI/CE</div>
						</div>
					</div>
	...
	<script>
		...
		function fnActualizarTabla() {
    		...
    		detalleTabla += "<td>" + (item.names || "") + "</td>"; // Verificación de names
    		detalleTabla += "<td>" + (item.last_names || "") + "</td>"; // Verificación de last_names
    		detalleTabla += "<td>" + (item.document_type || "") + "</td>"; // Verificación de document_type
    		detalleTabla += "<td>" + (item.document_number || "") + "</td>"; // Verificación de document_number
    		detalleTabla += "<td>" + (item.email || "") + "</td>"; // Verificación de email
    		detalleTabla += "<td>" + (item.cell_phone || "") + "</td>"; // Verificación de cell_phone
    		detalleTabla += "<td>" + (item.status || "") + "</td>"; // Verificación de status
    		detalleTabla += "<td>";
		...
		function fnBtnProcesar() {
			...
			xhr.open("POST", "managerProcesar", true);
			...
			xhr.onreadystatechange = function() {
				if (xhr.readyState === 4 && xhr.status === 200) {
					...
					window.location.href = "manager2.jsp";
		...
		function fnBtnBuscar() {
			...
			// Preparar la URL
			let url = "managerBuscar2?last_names=" + last_names + "&names="
					+ names;
			// La llama AJAX
		...
		function toggleStatus(managerId) {
			if (confirm("¿Estás seguro de que deseas activar/desactivar este encargado?")) {
				let xhr = new XMLHttpRequest();
				xhr.open("POST", "managerToggleStatus", true);
				...
				xhr.onreadystatechange = function() {
					if (xhr.readyState === 4 && xhr.status === 200) {
						...
						window.location.href = "managerBuscar2";// Redirigir a la página actual (recargar)
					}
				};
		...
		function generarPDF() {
			...
			let xhr = new XMLHttpRequest();
			xhr.open("POST", "GenerarPDFmanager", true); // Reemplaza "generarPDF" con la URL que maneje la generación del PDF
		...
		function generarCSV() {
			...
			let xhr = new XMLHttpRequest();
			xhr.open("POST", "GenerarCSVmanager", true); // Reemplaza "GenerarCSVstudent" con la URL que maneje la generación del CSV
			xhr.setRequestHeader('Content-type', 'application/json');
			xhr.responseType = "blob";
			xhr.onload = function() {
				if (xhr.status === 200) {
		...
		function generarXLS() {
			...
			let xhr = new XMLHttpRequest();
			xhr.open("POST", "GenerarXLSmanager", true); // Reemplaza "GenerarXLSattorney" con la URL que maneje la generación del XLS
			xhr.setRequestHeader('Content-Type', 'application/json');
			xhr.responseType = "blob";
			xhr.onload = function() {
				if (xhr.status === 200) {
