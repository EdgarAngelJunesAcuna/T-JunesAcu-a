...
public class CrudManager implements CrudServiceSpec<Manager>, RowMapper<Manager> {

	// Definiendo cosas
	private final String SQL_SELECT_BASE = "SELECT id id, names, last_names, document_type, email, document_number, cell_phone, status FROM manager ";
	private final String SQL_INSERT = "INSERT INTO manager(names,last_names,document_type,email,document_number,cell_phone,status) VALUES(?,?,?,?,?,?,?)";
	private final String SQL_UPDATE = "UPDATE manager SET names=?,last_names=?,document_type=?,email=?,document_number=?,cell_phone=?,status=? WHERE id = ?";
	private final String SQL_DELETE = "DELETE FROM manager WHERE id=?";
	// Borrado logico (De Activo a Inactivo y viceversa)
	private final String SQL_TOGGLE_STATUS = "UPDATE manager SET status = CASE WHEN status = 'A' THEN 'I' ELSE 'A' END WHERE id = ?";
	@Override
	...
	@Override
	public Manager getForId(String id) {
		// Variables
		Connection cn = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
	 ...
	 */
	@Override
	public List<Manager> get(Manager bean) {
		// Variables
		Connection cn = null;
		List<Manager> lista = new ArrayList<>();
		PreparedStatement pstm = null;
	...
	@Override
	public void insert(Manager bean) {
		// Variables
		Connection cn = null;
		PreparedStatement pstm = null;
		Integer id = 0;
	...
	public void delete(String id) {
	    ...
	    try {
	        ...
	        // Eliminar el encargado de la tabla
	...
	public void deleteByStatus(String id) {
	    ...
	    try {
	        ...
	        // Manejar excepciones
	        throw new RuntimeException("Error al cambiar el estado del encargado: " + e.getMessage());
	    } finally {
	        // Cerrar recursos
	        try {
	            if (pstm != null) {
